...
-- These ensure strict data integrity and prevent "magic strings" in code.
DO $$ 
BEGIN 
    CREATE TYPE user_role AS ENUM ('GUEST', 'BUYER', 'VENDOR', 'ADMIN', 'SUPER_ADMIN');
    ...
    CREATE TYPE vault_status AS ENUM ('LOCKED', 'RELEASE_PENDING', 'RELEASED', 'FROZEN'); 
EXCEPTION 
...
-- Escrow Ledger (The Double-Entry Vault)
...
DROP TRIGGER IF EXISTS trigger_release_timer ON public.orders; 
CREATE TRIGGER trigger_release_timer
BEFORE UPDATE ON public.orders
FOR EACH ROW EXECUTE FUNCTION set_release_timer();

-- 4. SUPER ADMIN VIEWS (THE FRIDAY BATCH)
-- This view generates the "Friday Payout Queue" for the Super Admin.
-- It only shows orders that are DELIVERED, past the 48h buffer, and NOT disputed.
...
-- Standard RLS: Users only see what belongs to them
CREATE POLICY "Users view own profile" ON public.profiles FOR SELECT USING (auth.uid() = id); 
CREATE POLICY "Public product view" ON public.products FOR SELECT USING (is_active = true); 
CREATE POLICY "Users view own orders" ON public.orders FOR SELECT USING (auth.uid() = buyer_id OR auth.uid() = vendor_id);
